"""05: A1 commissioning — R-013 (relearning-savings instrument on known arms).
...
OFFSETS = list(range(0, 401, 10)) + list(range(450, 1601, 50))
...
def alias(arm, oname, seed, rerun=False):
    ...
    return f"{RUNS_DIR}/comm_a1_{arm}_{oname}_s{seed}{tag}.jsonl"
...
def cell_times(curve1, curve2, channel):
    ...
    t1 = next((o for o in OFFSETS if c1[o] >= bar), None)
    if t1 is None or t1 == 0:
        return ("excluded", None, None)
    t2 = next((o for o in OFFSETS if c2[o] >= bar), None)
    censored = t2 is None
    if censored:
        t2 = PHASE
...
def analyze():
    lines = ["R-013 A1 COMMISSIONING ANALYSIS", "=" * 50]
...
if __name__ == "__main__":
    ...
    print(text)
    with open(f"{RUNS_DIR}/commissioning_a1_verdict.txt", "w") as f:
        f.write(text + "\n")
