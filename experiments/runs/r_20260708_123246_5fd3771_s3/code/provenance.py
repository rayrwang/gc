...
Usage:
    from experiments.provenance import stamp, run_id, write_stamped, load_stamped
...
CHARTER = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CHARTER.md")
...
def stamp(seed, extra_files=()):
    ...
    return {
        ...
        "experiments_hash": _hash_files(experiment_files()),
...
def snapshot_code(run_dir, extra_files=()):
    ...
    for p in list(experiment_files()) + list(extra_files):
...
def write_stamped(path, stamp_dict, step=None):
    ...
    if os.path.exists(path) and not os.path.islink(path):
        raise FileExistsError(f"refusing to overwrite evidence: {path} "
                              "(runs are append-only history; use a fresh run dir)")
...
def selftest():
    ...
    assert len(s["experiments_hash"]) == 12 and len(s["extra_hash"]) == 12
    import tempfile
    rd = tempfile.mkdtemp()
