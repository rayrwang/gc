...
def class_group_tasks(seed, groups, dataset="mnist"):
    """One Task per class-group, e.g. groups=((0,1),(2,3),...): the R-007
    5x2 curriculum. Shuffled single-pass streams, deterministic per seed."""
    loader, in_dim, n_classes = DATASETS[dataset]
    X, Y = loader(train=True)
    tasks = []
    for g in groups:
        name = "".join(map(str, g))
        mask = torch.isin(Y, torch.tensor(g))
        Xc, Yc = X[mask], Y[mask]
        perm = torch.randperm(len(Xc), generator=gen(seed, f"stream_g{name}"))
        samples = [(Xc[i], _onehot(Yc[i], n_classes)) for i in perm.tolist()]
        tasks.append(Task(f"{dataset}-g{name}", samples))
    return tasks
...
def selftest():
    ...
    gs = class_group_tasks(seed=0, groups=((0, 1), (2, 3)))
    assert len(gs) == 2 and {int(y.argmax()) for _, y in gs[0].samples[:100]} <= {0, 1}
