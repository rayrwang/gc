...
def rdm_profiles(Rtr, Ytr):
    """Decoder-free geometry (R-011): per-class centroids -> 10x10 pairwise
    RDM -> upper triangle. Two variants: euclidean + correlation distance."""
    labels = Ytr.argmax(1)
    cents = []
    for c in range(Ytr.shape[1]):
        m = labels == c
        cents.append(Rtr[m].mean(0) if bool(m.any()) else torch.zeros(Rtr.shape[1]))
    C = torch.stack(cents)                            # (n_classes, d)
    de = torch.cdist(C, C)
    Cc = C - C.mean(1, keepdim=True)
    Cn = Cc / (Cc.norm(dim=1, keepdim=True) + 1e-9)
    dc = 1 - Cn @ Cn.T                                # correlation distance
    iu = torch.triu_indices(C.shape[0], C.shape[0], offset=1)
    return {"rdm_euclid": [round(float(v), 5) for v in de[iu[0], iu[1]]],
            "rdm_corr":   [round(float(v), 5) for v in dc[iu[0], iu[1]]]}
            ...
            for i in range(n):
                ...
                if step % probe_every == 0:
                    ...
                    if len(pte) >= 10:
                        ...
                        Rtr_g = _collect_reps(arm, ptr)
                        row.update(rdm_profiles(Rtr_g, torch.stack([y for _, y in ptr])))
                        if stored_probes:
