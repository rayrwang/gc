...
FINDINGS (2026-07-08, 64 runs r_120700..124906, verdict FAIL -> R-008):
Gate (b) 6/6 clean. Gate (a) 0/3: drift-S ~= 1.0 — SATURATION, the opposite
failure to R-006: stale probes collapse to chance by the gate (fit@1600 ->
0.118 at 8000), order-independent churn swamps the differential. Post-hoc
short-lag (unsaturated, 0.28-0.59): still 1/3 (kNN p~5e-4 only). Twice-
evidenced finding: shallow batch-1 MLP reps are order-insensitive relative
to their own churn. Pre-registered escalation -> deeper arm (R-009).
"""
import argparse
import itertools
import json
