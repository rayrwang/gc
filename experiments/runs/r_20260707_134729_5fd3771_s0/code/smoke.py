"""Smoke run (pre-registration free play, §5): all three arms on real MNIST
...
import torch
from battery.harness import SGDArm, RandomProjArm, mnist_hebb_arm, run_curriculum
from battery.tasks import digit_split_tasks, probe_sets
...
def main():
    ...
    os.makedirs("battery/runs", exist_ok=True)
    for tag, factory in arms.items():
        out = run_curriculum(factory, sched, seed=0,
                             out_path=f"battery/runs/smoke_{tag}.jsonl",
