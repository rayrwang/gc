...
if __name__ == "__main__":
    ...
    if args.load:
        ...
        AGT_PATH = f"{project_root_path}/saves/mnist_active_agt-{datetime.datetime.now().astimezone().strftime("%Y%m%d_%H%M%S")}"
        agt = BareAgt(BareCfg(N_COLS, ispec, ospec), AGT_PATH)
    agt.debug_init()

    t_start = datetime.datetime.now().astimezone()
    for step in itertools.count():
        t_now = datetime.datetime.now().astimezone()
        print(f"\n{t_now} (Age: {agt.age} | Session: step {step}, elapsed {t_now-t_start})")
