...
def debugger(PATH, pipes):
    ...
    def handle_events():
        ...
        if gui_state["loc"] and buttons[2]:  # Right click on a selected col
    ...
    def draw_detail():
        ...
        if not gui_state["loc"]:  # No col selected
            return
        loc = gui_state["loc"]
        if os.path.isdir(f"{PATH}/{loc}"):  # Check if col location is valid
            loc = draw_col_detail(loc)
            if gui_state["conn"] is not None:
                draw_conn_detail(loc, gui_state["conn"])
            if gui_state["atv"] is not None:
                draw_atv(loc, gui_state["atv"])
